...
def add_vs_team_total(config: dict, year: int, person: dict):
    ...
    vs_teams = [
        StatsAPI.Person.person(
            ...
            query_params={"hydrate": hydrate.format(group=group, opposingTeamId=vs_team_id, season=year - 1)}
        ).get().obj["people"][0]
...
def build(brand: str, year: int, person_id: int):
    ...
    return card_class(year, person_id, *boxes, name_slug=person["nameSlug"], images=config["images"])

...
from .env import STATS_API_OBJECT__BASE_FILE_PATH, STATS_API_OBJECT__MAX_GET_TRIES
...
class StatsAPIObject(LogMixin):
    ...
    base_file_path = STATS_API_OBJECT__BASE_FILE_PATH
    max_get_tries = STATS_API_OBJECT__MAX_GET_TRIES

    def __init__(self, endpoint, api, operation, path_params=None, query_params=None):
        super(StatsAPIObject, self).__init__()
        self.endpoint = endpoint
        self.api = api
        self.operation = operation
        self.path_params, self.query_params = dict((path_params or {}).items()), dict(
            (query_params or {}).items()
        )
    ...
    def get(self, tries: int = 0):
        try:
            ...
            self.obj.pop("copyright") if isinstance(self.obj, dict) and (
                "copyright" in self.obj.keys()
            ) else ()
...
def resolve_path(api, operation, path_params=None, query_params=None):
    ...
    for path_param in operation.path_params:
        ...
        assert not (path_param.required and (param_name not in path_params)), (
            "path_param %s is required" % param_name
        )
        if param_name in path_params:
            arg_val = path_params.pop(param_name)
            if isinstance(arg_val, list):
                assert len(arg_val) == 1, "Multiple %s not allowed, found %s" % (
                    param_name,
                    str(arg_val),
                )
                arg_val = arg_val[0]
            ...
            if path_param.enum is not None:
                ...
                assert (
                    arg_val in enums
                ), "Unrecognized %s %s choice:'%s'. Please choose from [%s]" % (
                    operation.nickname,
                    param_name,
                    arg_val,
                    ", ".join(enums),
    ...
    assert (
        not path_params
    ), "Unrecognized %s path_params: '%s'. Please choose from [%s]" % (
        operation.nickname,
        "', '".join(path_params.keys()),
        ", ".join([p.name for p in operation.path_params]),
    )
    ...
    path = os.path.join(
        ...
        *[
            value
            for param, value in fmt_path_params.items()
            if (r"{%s}" % param) not in path
        ],
    ).format(
        **{
            param: value
            for param, value in fmt_path_params.items()
            if (r"{%s}" % param) in path
        }
    )
    ...
    for query_param in operation.query_params:
        ...
        ), "%s query_param '%s' is required!" % (operation.nickname, param_name)
        if param_name in query_params:
            arg_val = query_params.pop(param_name)
            if isinstance(arg_val, list):
                is_valid = (len(arg_val) == 1) or query_param.allowMultiple
                assert is_valid, f"multiple {param_name} not allowed, got %s" % arg_val
    ...
    assert (
        not query_params
    ), "Unrecognized %s query_params: '%s'. Please choose from [%s]" % (
        operation.nickname,
        "', '".join(query_params.keys()),
        ", ".join([qp.name for qp in operation.query_params]),
    )
    queries = [
        f"{p}={v}" for p, v in sorted(fmt_query_params.items(), key=lambda x: x[0])
    ]
    if queries:
        path += "?" + "&".join(queries)
...
def configure_api(func):
    ...
    def wrapper(*args, **kwargs):
        path_in_args = arg_path in func_params and func_params.index(arg_path) < len(
            args
        )
